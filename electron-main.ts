...
function isTrustedBackendHost(hostname: string) {
...
function localSafetyCheck(prompt: string) {
...
function withStoreSafetyNegativePrompt(negativePrompt: unknown) {
...
function runStableDiffusionCli(exePath: string, args: string[], cwd: string, expectedSteps: number, env?: NodeJS.ProcessEnv) {
  ...
  return new Promise((resolve) => {
    exec('nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits', { windowsHide: true }, (err, stdout) => {
      if (!err && stdout) {
        ...
        if (!Number.isNaN(val)) return resolve(Math.max(0, Math.min(100, val)));
      }
